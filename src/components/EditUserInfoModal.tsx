...
const EditUserInfoModal = ({ user, onCancel, onUpdate }) => {
  const {
    register,
    handleSubmit,
    watch,
  ...
  const onFileChange = (event) => {
    if (event.target.files && event.target.files[0]) {
      ...
      reader.onload = (e) => {
        setImagePreview(e.target.result); // 이미지 URL을 상태에 저장
      };
    ...
    <Modal>
      ...
      <Form onSubmit={handleSubmit(onSubmit)}>
        <ProfileImageBox>
          ...
          <UploadLabel htmlFor="upload-button"><UploadIcon /></UploadLabel>
          <Input
            id="upload-button"
            type="file"
            accept="image/*"
